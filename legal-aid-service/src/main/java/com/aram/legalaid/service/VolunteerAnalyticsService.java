...
public class VolunteerAnalyticsService {
    ...
    public VolunteerAnalyticsService(LegalGuideProfileRepository guideProfileRepository, 
    ...
    public Map<String, Object> getVolunteerAnalytics(Long volunteerId) {
        com.aram.legalaid.model.LegalGuideProfile guide = guideProfileRepository.findByUserId(volunteerId)
                .orElseThrow(() -> new com.aram.legalaid.exception.ResourceNotFoundException("Volunteer not found"));

        User volunteer = new User(guide.getUserId(), guide.getFullName(), guide.getEmail(), Role.HELPER);

        List<Complaint> cases = complaintRepository.findByAssignedHelperIdOrderByCreatedAtDesc(volunteerId);
        List<VolunteerActivityLog> activities = activityRepository.findByVolunteerIdOrderByCreatedAtDesc(volunteerId);
        LegalGuidePerformanceProfile perf = levelService.getOrCreatePerformanceProfile(volunteerId);
        ...
        volInfo.put("experienceLevel", perf.getCurrentLevelName());
        volInfo.put("levelName", perf.getCurrentLevelName());
        volInfo.put("levelNumber", perf.getCurrentLevelNumber());
        volInfo.put("creditScore", perf.getCreditScore());
        volInfo.put("yearsExperience", guide.getExperienceYears());
        ...
        summary.put("rank", 3);
        summary.put("totalVolunteers", guideProfileRepository.count());
