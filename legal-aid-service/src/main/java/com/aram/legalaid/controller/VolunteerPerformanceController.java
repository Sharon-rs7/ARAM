...
public class VolunteerPerformanceController {
    ...
    public ResponseEntity<Map<String, String>> awardCredits(
        ...
        if (body.get("points") == null) {
            throw new com.aram.legalaid.exception.BadRequestException("Credit points are required");
        }
    ...
    public ResponseEntity<Map<String, Object>> getGuideTrustSummary(@PathVariable Long complaintId) {
        ...
        Optional<LegalGuideProfile> guideProfileOpt = guideProfileRepository.findByUserId(guide.getId());
        if (guideProfileOpt.isPresent()) {
            LegalGuideProfile profile = guideProfileOpt.get();
            summary.put("verified", "APPROVED".equalsIgnoreCase(profile.getVerificationStatus()) || "VERIFIED".equalsIgnoreCase(profile.getVerificationStatus()));
            summary.put("languages", profile.getLanguagesKnown() != null ? List.of(profile.getLanguagesKnown().split(",")) : List.of("English", "Tamil"));
            summary.put("specialization", profile.getExpertiseCategories());
        } else {
            summary.put("verified", false);
            summary.put("languages", List.of("English", "Tamil"));
            summary.put("specialization", "");
        }
