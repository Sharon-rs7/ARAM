...
public class HelperController {
    ...
    private static final String AUTH_SERVICE_ME_URL = "http://localhost:8081/api/users/me";

    public HelperController(UserService userService, MapperService mapperService,
                            ComplaintRepository complaintRepository, AIResultRepository aiResultRepository,
                            ComplaintService complaintService,
                            com.aram.legalaid.service.LegalGuideLevelService legalGuideLevelService) {
    ...
    public ResponseEntity<List<ComplaintResponse>> cases() {
        ...
        return ResponseEntity.ok(complaintRepository.findByAssignedHelperIdOrderByCreatedAtDesc(helper.getId()).stream()
                .map(this::toResponse)
                .toList());
    }
    ...
    public ResponseEntity<ComplaintResponse> caseDetails(@PathVariable Long id) {
        ...
        if (complaint.getAssignedHelperId() == null || !complaint.getAssignedHelperId().equals(helper.getId())) {
    ...
    public ResponseEntity<java.util.Map<String, Object>> getDashboard() {
        User helper = requireHelper();
        List<Complaint> cases = complaintRepository.findByAssignedHelperIdOrderByCreatedAtDesc(helper.getId());
        ...
        volunteerInfo.put("verified", true);
        volunteerInfo.put("availabilityStatus", "AVAILABLE");
        volunteerInfo.put("languages", List.of("English", "Tamil"));
        volunteerInfo.put("specializations", List.of("Labour Rights", "General Legal Aid"));
        volunteerInfo.put("currentActiveCases", cases.size());
        volunteerInfo.put("maxActiveCases", 5);
