...
const CitizenSidebar = () => {
  ...
  const [topHeight, setTopHeight] = useState(() => {
    const saved = localStorage.getItem("sidebar_citizen_top_height");
  ...
  useEffect(() => {
    const handleMouseMove = (mouseMoveEvent) => {
      ...
      if (!rect) return;
      // Restrict heights to stay within bounds
      const newHeight = Math.max(100, Math.min(rect.height - 120, mouseMoveEvent.clientY - rect.top));
      setTopHeight(newHeight);
      localStorage.setItem("sidebar_citizen_top_height", newHeight.toString());
    };
