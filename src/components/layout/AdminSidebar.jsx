...
const AdminSidebar = () => {
  ...
  const [topHeight, setTopHeight] = useState(() => {
    const saved = localStorage.getItem("sidebar_admin_top_height");
  ...
  useEffect(() => {
    const handleMouseMove = (mouseMoveEvent) => {
      ...
      if (!rect) return;
      const newHeight = Math.max(120, Math.min(rect.height - 120, mouseMoveEvent.clientY - rect.top));
      setTopHeight(newHeight);
      localStorage.setItem("sidebar_admin_top_height", newHeight.toString());
    };
