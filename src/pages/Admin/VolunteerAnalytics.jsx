...
export default function VolunteerAnalytics() {
  ...
  const loadData = async () => {
    ...
    setError("");
    try {
      const analytics = await volunteerService.getVolunteerAnalytics(id);
      setData(analytics);
    } catch (err) {
      console.error("Failed to load volunteer analytics", err);
      setError("Unable to load volunteer analytics data. Verify the backend service is active.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <DashboardLayout>
        <div className="flex h-[60vh] items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <p className="text-slate-500 text-sm font-semibold">Loading volunteer performance profile...</p>
          </div>
        </div>
      </DashboardLayout>
    );
      ...
      <DashboardLayout>
        <div className="flex h-[60vh] items-center justify-center">
          <div className="text-center max-w-md p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
            ...
            >
              <ChevronLeft size={14} /> Back to Legal Guides
            </button>
          </div>
        </div>
      </DashboardLayout>
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Legal Guide Analytics</h1>
            <p className="text-xs text-slate-500">LeetCode-style profile metrics & activity overview</p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-150">
            ...
            <p className="text-xs text-slate-500 mt-1">Cases matched by triage</p>
          </div>

          <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-150">
            <div className="flex justify-between items-center text-green-600">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Resolved Cases</span>
              <CheckCircle2 size={20} />
            </div>
            <h3 className="mt-2 text-2xl font-black text-slate-800">{summary.resolved}</h3>
            <p className="text-xs text-slate-500 mt-1">Successful mediation actions</p>
          </div>
          ...
          <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-150">
            <div className="flex justify-between items-center text-red-500">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Resolution Success</span>
              <TrendingUp size={20} />
            </div>
            <h3 className="mt-2 text-2xl font-black text-slate-800">{summary.successRate}%</h3>
            <p className="text-xs text-slate-500 mt-1">Target SLA threshold compliance</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-150 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Legal Specializations</h4>
                ...
                <div>
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Languages Handled</h4>
                  <div className="flex flex-wrap gap-1.5">
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Safety Credentials</h4>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              {/* Badges Card */}
              <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-150 flex flex-col justify-between">
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-3">Legal Guide Badges</h3>
                <div className="grid grid-cols-3 gap-3 overflow-y-auto max-h-[120px] pr-1">
                  {badges.map((badge, idx) => (
                    ...
                    >
                      <Award size={26} className={badge.status === "UNLOCKED" ? "text-indigo-600 animate-bounce-short" : "text-slate-350"} />
                ...
                {badges.some(b => b.status === "LOCKED") && (
                  <p className="text-[10px] text-slate-400 mt-2 font-semibold">
                    🔑 Complete more verified case actions to unlock locked badges.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-150 space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <Flame size={16} className="text-orange-500 animate-pulse" /> Activity Heatmap
                </h3>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-150">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-4 flex items-center gap-1.5">
                  <TrendingUp size={14} className="text-indigo-600" /> Cases by Category
                </h4>
              ...
              <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-150">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-4">Priority Distribution</h4>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-150">
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">SLA Performance Benchmarks</h3>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 font-semibold">
                    <tr className="hover:bg-slate-50/50">
                      <td className="py-2.5 text-slate-700">Average Case Response Time</td>
                      <td className="py-2.5 text-slate-800 font-mono">42 minutes</td>
                      <td className="py-2.5 text-slate-400">Under 60 minutes</td>
                    ...
                    <tr className="hover:bg-slate-50/50">
                      <td className="py-2.5 text-slate-700">SLA Compliance Resolution Rate</td>
                      <td className="py-2.5 text-slate-800 font-mono">88%</td>
                      <td className="py-2.5 text-slate-400">At least 85%</td>
                      <td className="py-2.5 text-right"><span className="px-2 py-0.5 rounded bg-green-50 text-green-700 font-bold uppercase text-[9px] border border-green-100">Compliant</span></td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="py-2.5 text-slate-700">Resolution Mediation Rate</td>
                      <td className="py-2.5 text-slate-800 font-mono">{summary.successRate}%</td>
                      <td className="py-2.5 text-slate-400">At least 90%</td>
                      <td className="py-2.5 text-right"><span className="px-2 py-0.5 rounded bg-green-50 text-green-700 font-bold uppercase text-[9px] border border-green-100">Compliant</span></td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="py-2.5 text-slate-700">Active Workload Capacity Utilization</td>
                      <td className="py-2.5 text-slate-800 font-mono">{Math.round(((summary.inProgress + summary.pending) / (volunteer.maxActiveCases || 5)) * 100)}%</td>
                      <td className="py-2.5 text-slate-400">Under 100% capacity</td>
                      <td className="py-2.5 text-right"><span className="px-2 py-0.5 rounded bg-green-50 text-green-700 font-bold uppercase text-[9px] border border-green-100">Safe Workload</span></td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-150 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-150 pb-3">
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <FileText size={16} className="text-slate-400" /> Recent Case Actions
                </h3>
                {/* Tabs */}
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {filteredActivities.length === 0 ? (
                  <p className="text-xs text-slate-400 text-center py-6 font-semibold">No recent activity matching filter</p>
                ) : (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <strong className="text-xs text-slate-800">{act.caseId}</strong>
