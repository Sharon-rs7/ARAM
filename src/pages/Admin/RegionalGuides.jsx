...
export default function RegionalGuides() {
  ...
  const loadGuides = async () => {
    ...
    setError(null);
    try {
      const data = await regionalAdminService.getGuides(district);
      setGuides(data || []);
    } catch (err) {
      console.error(err);
      setError("Failed to fetch legal guides listing.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#18332B] tracking-tight">
              Legal Guides & Volunteers Directory
            </h1>
            <p className="text-xs text-[#65736D] mt-1 font-medium">
              Verified paralegals, advocates, and dispute mediators active in {district}.
            </p>
          </div>
          ...
          >
            <RefreshCw size={14} className={loading ? "animate-spin" : ""} /> Refresh Guides
          </button>
        </div>
